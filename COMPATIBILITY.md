...
- runtime overrides: `PLATFORM_METADATA_URL`, `PLATFORM_METADATA_ADDRESS`, and `PLATFORM_CA_ROOT`, constrained to the two reserved platform link-local metadata addresses, fixed API path, standard ports, and managed certificate directory;
- optional `logToFile` paths beneath `/var/log/pasturestack`, plus the existing `/var/log/pasturestack-cni.log` deployment path.
